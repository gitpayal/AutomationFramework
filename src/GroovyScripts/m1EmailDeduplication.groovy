...
public void m1emaildeduplication() {
...
int countexception =0;
 ...
 WebDriver driver;
   ...
   String URL = "Mailpayalverma@gmail.com";
...
//sleep(1000)
 ...
 FileInputStream fIpStream= new FileInputStream(new File("./TestReport.xls")); 

                    HSSFWorkbook wb = new HSSFWorkbook(fIpStream);
                      // accessing your existing sheet
                    HSSFSheet worksheet = wb.getSheetAt(0);

                    Row row = worksheet.createRow(140);
                    Cell cell = row.createCell(2);

                    cell.setCellValue("De duplication test for Signup Automation is completed ");
                      //cell.setCellValue(fieldFromResponse)
                    fIpStream.close(); //Close the InputStream

                    FileOutputStream output_file =new FileOutputStream(new File("./TestReport.xls"));  
...
sleep(1000)
 ...
 String nrictestdata = nric_generator();
    ...
    driver.findElement(By.id("merchant_business_person_attributes_nric")).sendKeys(nrictestdata);
    sleep(1000)
...
driver.findElement(By.id("merchant_bank_branch_code")).sendKeys(Keys.chord(Keys.CONTROL, "a"),"100"); 


sleep(1000)
driver.findElement(By.id("merchant_bank_branch_code_confirmation")).sendKeys(Keys.chord(Keys.CONTROL, "a"),"100");
...
if (driver.findElement(By.cssSelector("div[class = 'input email required merchant_email'] span[class='error']")).isDisplayed())
{
     ...
     String duplicationerror = driver.findElement(By.cssSelector("div[class = 'input email required merchant_email'] span[class='error']")).getText();
	System.out.println("De duplicaton test case for duplicate email is completed." +duplicationerror);
}
else
{
	System.out.println("De duplicaton test case for duplicate email failed.");
	}
}
	  ...
	  if(driver.findElement(By.cssSelector("div[class = 'input string optional merchant_company_name']")).isDisplayed())
{
	...
	if(driver.findElement(By.cssSelector("div[class = 'input string optional merchant_company_name'] span[class='error']")).isDisplayed())
                     ...
                     {
                     cnfserror = driver.findElement(By.cssSelector("div[class = 'input string optional merchant_company_name'] span[class='error']")).getText();
                     System.out.println("merchant bank account number format is not correct :" +cnfserror)
                     }
                    else
                     {
                           System.out.println("merchant company name validation failed");
